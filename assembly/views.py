...
def convert_audio_to_text(request, pk):
    ...
    file.save()
    txt_file_path = os.path.join(settings.MEDIA_ROOT, f'{os.path.splitext(file.file.name)[0]}.txt')
...
def download_text_file(request, pk):
    ...
    if os.path.exists(txt_file_path):
        with open(txt_file_path, 'rb') as f:
            ...
            response['Content-Disposition'] = f'attachment; filename="{txt_file_name}"'
            return response
    else:
